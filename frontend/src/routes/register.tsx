...
export const sendRegistrationOtp = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (dbError) {
      ...
      throw new Error("Failed to generate OTP. Ensure otp_verifications table exists.");
    }
...
export const verifyAndCreateUser = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const hashed_input = crypto.createHash('sha256').update(otp).digest('hex');
    if (otpRecord.otp_hash !== hashed_input) throw new Error("Invalid verification code.");
    ...
    await supabaseAdmin.from('profiles').insert({ id: authData.user.id, display_name: name });
