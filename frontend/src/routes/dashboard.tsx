...
function DashboardPage() {
  ...
  // ── Fetch Dashboard Data from ML Backend ──────────────────────────────────────
  const fetchDashboardData = useCallback(async (token: string) => {
    ...
    try {
      ...
      setProfile({
        ...
        updated_at: data.profile?.updated_at ?? null,
      });
      
      setTwinStatus(data.twin_status);
      
      if (data.predictions_history && data.predictions_history.length > 0) {
        setLatestPrediction(data.predictions_history[data.predictions_history.length - 1]);
        setPreviousPrediction(data.predictions_history.length > 1 ? data.predictions_history[data.predictions_history.length - 2] : null);
        setAllPredictions(data.predictions_history);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  }, [userId, authLoading, fetchDashboardData, fetchSleepData, fetchActiveSession]);
